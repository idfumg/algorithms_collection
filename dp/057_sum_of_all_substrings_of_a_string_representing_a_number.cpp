...
int count(string& s, int n, int i) {
    if (i < 0) return 0;
    return count(s, n, i - 1) + stoi(s.substr(i, n - i + 1));
}

int rec(string& s, int n) {
    if (n < 0) return 0;
    return rec(s, n - 1) + count(s, n, n);
}
...
int opt2(string& s) {
    ...
    int sum = 0;
    for (int i = 1; i <= n; ++i) {
        const int num = s[i - 1] - 48;
        dp[i & 1] = num * i + 10 * dp[1 - (i & 1)];
        sum += dp[i & 1];
    }
    return sum;
}
