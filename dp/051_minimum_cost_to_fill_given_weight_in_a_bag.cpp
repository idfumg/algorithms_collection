...
int rec(vi& arr, int W, int n) {
    if (W == 0) return 0;
    if (W < 0 or n < 0) return INF;
    const auto without = rec(arr, W, n - 1);
    if (arr[n] == -1) { // can't use it
        return without;
    }
    const auto with = rec(arr, W - n - 1, n) + arr[n];
    return min(without, with);
}

int rec(vi& arr, int W) {
    return rec(arr, W, arr.size() - 1);
}

int tab(vi& arr, int W) {
    const auto n = arr.size();
    vvi dp(W + 1, vi(n + 1, INF));
    for (int i = 0; i <= W; ++i) {
        for (int j = 1; j <= n; ++j) {
            if (i == 0) {
                dp[i][j] = 0;
                ...
                dp[i][j] = dp[i][j - 1];
                if (arr[j - 1] != -1 and i - j >= 0) {
                    dp[i][j] = min(dp[i][j], dp[i - j][j] + arr[j - 1]);
                }
            }
        }
    }
...
int main() { TimeMeasure _; __x();
    vi arr1 = {20, 10, 4, 50, 100}; int W1 = 5;
    vi arr2 = {1, 10, 4, 50, 100}; int W2 = 5;
    vi arr3 = {1, 2, 3, 4, 5}; int W3 = 5;
    vi arr4 = {-1, -1, 4, 5, -1}; int W4 = 5;
    cout << "Recursive: " << rec(arr1, W1) << endl;
    cout << "Recursive: " << rec(arr2, W2) << endl;
    cout << "Recursive: " << rec(arr3, W3) << endl;
    cout << "Recursive: " << rec(arr4, W4) << endl;
    cout << "Tabulation: " << tab(arr1, W1) << endl;
    cout << "Tabulation: " << tab(arr2, W2) << endl;
    cout << "Tabulation: " << tab(arr3, W3) << endl;
    cout << "Tabulation: " << tab(arr4, W4) << endl;
}
