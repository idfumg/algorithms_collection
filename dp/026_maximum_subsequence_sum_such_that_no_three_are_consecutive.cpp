...
int rec(vi& arr, int n) {
    if (n < 0) return 0;
    const auto a = rec(arr, n - 1);
    const auto b = rec(arr, n - 2) + arr[n];
    const auto c = rec(arr, n - 3) + arr[n] + arr[n - 1];
    return max({a, b, c});
}

int rec(vi& arr) {
    return rec(arr, arr.size() - 1);
}
...
int tab(vi& arr) {
    ...
    for (int i = 1; i <= arr.size(); ++i) {
        const auto a = dp[i - 1];
        const auto b = (i >= 2 ? dp[i - 2] : 0) + arr[i - 1];
        const auto c = (i >= 3 ? dp[i - 3] : 0) + arr[i - 1] + arr[i - 2];
        dp[i] = max({a, b, c});
    }
    return dp[arr.size()];
}
...
int main() { TimeMeasure _;
    vi arr = {1, 2, 3};
    vi arr2 = {3000, 2000, 1000, 3, 10};
    vi arr3 = {100, 1000, 100, 1000, 1};
    vi arr4 = {1, 1, 1, 1, 1};
    vi arr5 = {1, 2, 3, 4, 5, 6, 7, 8};

    cout << "Recursive: " << rec(arr) << endl;
    cout << "Recursive: " << rec(arr2) << endl;
    cout << "Recursive: " << rec(arr3) << endl;
    cout << "Recursive: " << rec(arr4) << endl;
    cout << "Recursive: " << rec(arr5) << endl;
    cout << endl;
    cout << "Tabulation: " << tab(arr) << endl;
    cout << "Tabulation: " << tab(arr2) << endl;
    cout << "Tabulation: " << tab(arr3) << endl;
    cout << "Tabulation: " << tab(arr4) << endl;
    cout << "Tabulation: " << tab(arr5) << endl;
}
