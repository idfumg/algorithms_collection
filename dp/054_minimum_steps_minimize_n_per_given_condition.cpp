...
int rec(int n) {
    if (n == 1) return 0;
    const int a = (n % 2 == 0) ? rec(n / 2) : INF;
    const int b = (n % 3 == 0) ? rec(n / 3) : INF;
    const int c = rec(n - 1);
    return min({a, b, c}) + 1;
}
...
int tab(int n) {
    vi dp(n + 1);
    for (int i = 2; i <= n; ++i) {
        const int a = (i % 2 == 0) ? dp[i / 2] : INF;
        const int b = (i % 3 == 0) ? dp[i / 3] : INF;
        const int c = dp[i - 1];
        dp[i] = min({a, b, c}) + 1;
    }
    return dp[n];
...
int main() { TimeMeasure _; __x();
    cout << "Recursive: " << rec(10) << endl;
    cout << "Recursive: " << rec(6) << endl;
    cout << "Tabulation: " << tab(10) << endl;
    cout << "Tabulation: " << tab(6) << endl;
}
