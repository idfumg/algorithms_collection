...
bool rec(vi& v, int m, int current, int i) {
    if (i < 0) return false;
    if (current % m == 0 and current != 0) return true;
    return rec(v, m, current + v[i], i - 1) or rec(v, m, current, i - 1);
}

bool rec(vi& v, int m) {
    return rec(v, m, 0, v.size() - 1);
}

bool tab(vi& v, int m) {
    for (int i = 0; i < v.size(); ++i) {
        for (int j = i + 1; j < v.size(); ++j) {
            if ((v[i] + v[j]) % m == 0) return true;
        }
    }
    return false;
}
...
int main() { TimeMeasure _;
    {
        vi v = {3, 1, 7, 5};
        int m = 6;
        cout << "Recursion: " << rec(v, m) << endl;
        cout << "Tabulation: " << tab(v, m) << endl;
    }
    {
        cout << endl;
        vi v = {1, 6};
