...
int count(vi& dp, int n, int i) {
    if (i < 0) return 0;
    if ((n + 1) % (i + 1) == 0) return max(dp[i], count(dp, n, i - 1));
    return count(dp, n, i - 1);
}

void rec(vi& dp, int n) {
    if (n < 1) return;
    rec(dp, n - 1);
    dp[n] = count(dp, n, n - 1) + dp[n];
}
...
int main() { TimeMeasure _; __x();
    vi arr = {2, 3, 1, 4, 6, 5};
    cout << "Recursive: "; rec(arr); cout << endl;
    cout << "Tabulation: "; tab(arr); cout << endl;
}
