...
int sliding_window(vi& arr, int k) {
    int wmax = 1, count = 0;
    for (int start = 0, end = 0; end < arr.size(); ++end) {
        wmax *= arr[end];

        while (start < end and wmax >= k) {
            wmax /= arr[start++];
        }
...
int main() { TimeMeasure _;
    vi arr = {1, 9, 2, 8, 6, 4, 3};
    int k = 100;
    cout << "Brute Force: " << bruteforce(arr, k) << endl;
    cout << "Sliding Window: " << sliding_window(arr, k) << endl;
}
