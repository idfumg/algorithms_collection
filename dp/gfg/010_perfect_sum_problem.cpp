...
void rec(vi& arr, int sum, int i, vi& numbers, vvb& dp) {
    if (sum == 0) {
        cout << numbers;
        return;
    }
    ...
    if (dp[sum][i]) {
        rec(arr, sum, i - 1, numbers, dp);
    }
    numbers.push_back(arr[i]);
    if (sum >= arr[i] and dp[sum - arr[i]][i]) {
        rec(arr, sum - arr[i], i - 1, numbers, dp);
    }
    numbers.pop_back();
}

void rec(vi& arr, int sum) {
    vi numbers;
    vvb dp(sum + 1, vb(arr.size() + 1));
    ...
    rec(arr, sum, arr.size() - 1, numbers, dp);
}

int main() { TimeMeasure _;
    vi arr1 = {2, 3, 5, 6, 8, 10};
    int sum1 = 10;
    vi arr2 = {1,2,3,4,5};
    int sum2 = 10;

    cout << "Recursive with DP: \n"; rec(arr1, sum1); cout << endl;
    cout << "Recursive with DP: \n"; rec(arr2, sum2); cout << endl;
}
