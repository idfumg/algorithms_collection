...
vi max_sum_increasing_subsequence(vi& v) {
    vi dp = v;
...
vi max_sum_decreasing_subsequence(vi& v) {
    vi dp = v;
...
int tab(vi& v) {
    vi increasing = max_sum_increasing_subsequence(v);
    vi decreasing = max_sum_decreasing_subsequence(v);

    int maxi = INT_MIN;
    for (int i = 0; i < v.size(); ++i) {
        maxi = max(maxi, increasing[i] + decreasing[i] - v[i]);
    }
    return maxi;
}

int main() { TimeMeasure _;
    vi v = {1, 15, 51, 45, 33, 100, 12, 18, 9};
    //cout << "Recursive: " << rec(v) << endl;
