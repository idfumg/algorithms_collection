...
int rec(int n, int a, int b, int c) {
    ...
    const auto x = rec(n - a, a, b, c);
    const auto y = rec(n - b, a, b, c);
    const auto z = rec(n - c, a, b, c);
    return max({x, y, z}) + 1;
}

int tab(int n, int a, int b, int c) {
    vi dp(n + 1, 0);
    dp[0] = 0;
    for (int i = 1; i <= n; ++i) {
        const auto x = i >= a ? dp[i - a] : INT_MIN;
        const auto y = i >= b ? dp[i - b] : INT_MIN;
        const auto z = i >= c ? dp[i - c] : INT_MIN;
        dp[i] = max({x, y, z}) + 1;
    }
    return dp[n];
}
...
int main() { TimeMeasure _; __x();
    cout << "Recursive: " << rec(7, 5, 2, 5) << endl;
    cout << "Recursive: " << rec(17, 2, 1, 3) << endl;
    cout << "Tabulation: " << tab(7, 5, 2, 5) << endl;
    cout << "Tabulation: " << tab(17, 2, 1, 3) << endl;
}
