...
bool isok(int m, int n, int i, int j) {
    return i >= 0 and j >= 0 and i <= m and j <= n;
}

int rec(vvi& mat, int m, int n, int i, int j) {
    if (not isok(m, n, i, j)) return INF;
    const auto a = rec(mat, m, n, i, j - 1);
    const auto b = rec(mat, m, n, i - 1, j);
    const auto c = rec(mat, m, n, i - 1, j - 1);
    const auto d = min({a, b, c});
    return (d == INF ? 0 : d) + mat[i][j];
}

int rec(vvi& mat) {
    const auto m = mat.size() - 1;
    const auto n = mat[0].size() - 1;
    return rec(mat, m, n, m, n);
}

int select(vvi mat, int m, int n, int i, int j) {
    if (not isok(m, n, i, j)) return INF;
    return mat[i][j];
}
...
int tab(vvi mat) {
    ...
    for (int i = 0; i <= m; ++i) {
        for (int j = 0; j <= n; ++j) {
            const auto a = select(mat, m, n, i - 1, j - 1);
            const auto b = select(mat, m, n, i, j - 1);
            const auto c = select(mat, m, n, i - 1, j);
            const auto d = min({a, b, c});
            mat[i][j] = (d == INF ? 0 : d) + mat[i][j];
        }
    }
    return mat[m][n];
}
