...
int rec(vi& arr, int n, int sum) {
    if (n < 0) return 0;
    const auto a = rec(arr, n - 1, sum + arr[n]);
    const auto b = rec(arr, n - 1, 0);
    return max({a, b, sum});
}
...
int main() { TimeMeasure _; __x();
    vi arr = {-2, -3, 4, -1, -2, 1, 5, -3}; // 7
    cout << "Recursive: " << rec(arr) << endl;
    cout << "Tabulation: " << tab(arr) << endl;
    cout << "Optimization: " << opt(arr) << endl;
    cout << "Optimization2: " << opt2(arr) << endl;
}
