...
bool satisfy_condition(int m, int n, int i, int j) {
    return i >= 0 and j >= 0 and i <= m and j <= n;
}

int rec(vvi& mat, int m, int n, int i, int j) {
    if (not satisfy_condition(m, n, i, j)) return 0;
    const auto a = rec(mat, m, n, i - 1, j - 1);
...
int tab(vvi mat) {
    ...
    for (int i = 0; i <= m; ++i) {
        for (int j = 0; j <= n; ++j) {
            ...
            const auto b = select(mat, m, n, i - 1, j);
            const auto c = select(mat, m, n, i - 1, j + 1);
            mat[i][j] = max({a, b, c}) + mat[i][j];
