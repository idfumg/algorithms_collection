...
int rec(vi& arr, int n) {
    if (n < 0) return 0;
    const auto a = rec(arr, n - 2) + arr[n];
...
int tab(vi& arr) {
    ...
    dp[1] = arr[0];
    for (int i = 2; i <= n; ++i) {
        const auto a = dp[i - 2] + arr[i - 1];
        const auto b = dp[i - 1] + 0;
        dp[i] = max(a, b);
    }
    return dp[n];
}
...
int opt(vi& arr) {
    ...
    dp[1] = arr[0];
    for (int i = 2; i <= n; ++i) {
        const auto a = dp[(i + 1) % 3] + arr[i - 1];
        const auto b = dp[(i + 2) % 3] + 0;
        dp[i % 3] = max(a, b);
    }
    return dp[n % 3];
}
