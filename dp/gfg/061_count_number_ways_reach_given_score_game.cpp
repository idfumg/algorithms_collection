...
int rec(vi& arr, int n, int i) {
    if (n == 0) return 1;
    if (i < 0 or n < 0) return 0;
    const auto a = rec(arr, n, i - 1);
    const auto b = rec(arr, n - arr[i], i);
    return a + b;
}

int rec(vi& arr, int n) {
    return rec(arr, n, arr.size() - 1);
}

int tab(vi& arr, int n) {
    vvi dp(arr.size() + 1, vi(n + 1));
    for (int i = 0; i <= n; ++i) {
...
int main() { TimeMeasure _; __x();
    vi arr = {3, 5, 10};
    cout << "Recursive: " << rec(arr, 20) << endl;
    cout << "Recursive: " << rec(arr, 13) << endl;
    cout << "Tabulation: " << tab(arr, 20) << endl;
    cout << "Tabulation: " << tab(arr, 13) << endl;
    cout << "Optimization: " << opt(arr, 20) << endl;
    cout << "Optimization: " << opt(arr, 13) << endl;
}
