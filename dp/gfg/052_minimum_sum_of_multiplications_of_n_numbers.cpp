...
vi make_arr(vi& arr, int i, int j) {
    if (i < 0 or j < 0 or i > j) return {};
    return vi(arr.begin() + i, arr.begin() + j + 1);
}

int rec(vi arr, int i, int j) {
    cout << arr;
    if (arr.empty() or i < 0 or j < 0) return 0;
    if (arr.size() == 2) return arr[0] * arr[1];

    const auto val = arr[j] * arr[j - 1];
    auto x = make_arr(arr, i, j - 1);
    auto a = rec(x, 0, x.size() - 1);
    x.push_back((arr[j] + arr[j - 1]) % 100);
    return rec(x,
}
...
int main() { TimeMeasure _; __x();
    //vi arr1 = {5, 6}; //30
    vi arr2 = {40, 60, 20}; // 2400
    // vi arr3 = {100, 200, 40}; // 12000
    // vi arr4 = {100, 100}; // 10000
    // vi arr5 = {1, 2, 3}; // 11
