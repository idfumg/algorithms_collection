...
int rec(Point X, Point Y, Point Z, int A, int B, vi can) {
    if (A <= 0 or B <= 0) return -1;
    if (dp[A][B] != -1) return dp[A][B];

    int x = INT_MIN, y = INT_MIN, z = INT_MIN;

    if (can[0]) x = rec(X, Y, Z, A + X.a, B + X.b, {false, true, true});
    if (can[1]) y = rec(X, Y, Z, A + Y.a, B + Y.b, {true, false, true});
    if (can[2]) z = rec(X, Y, Z, A + Z.a, B + Z.b, {true, true, false});

    return dp[A][B] = max({x, y, z}) + 1;
}

int rec(Point X, Point Y, Point Z, int A, int B) {
    return rec(X, Y, Z, A, B, {true, true, true});
}

int main() { TimeMeasure _;
    Point X{3, 2}, Y{-5, -10}, Z{-20, 5};
    int A = 20, B = 8;

    cout << "Recursive: " << rec(X, Y, Z, A, B) << endl;
}
