...
int change_rec(const vi& coins, int n, int coin) {
    if (n == 0) return 1;
    if (n < 0 or coin < 0) return 0;
    return
        change_rec(coins, n - coins[coin], coin) +
        change_rec(coins, n, coin - 1);
}

int change_rec(int n, const vi& coins) {
    return change_rec(coins, n, coins.size() - 1);
}

int change_tab(int n ,const vi& coins) {
    vvi dp(n + 1, vi(coins.size() + 1));
    ...
    for (int amount = 1; amount <= n; ++amount) {
        for (int coin = 0; coin <= coins.size(); ++coin) {
            dp[amount][coin] =
                (amount >= coins[coin] ? dp[amount - coins[coin]][coin] : 0) +
                dp[amount][coin - 1];
        }
    }
...
int main() { TimeMeasure _;
    const int n = 10;
    const vi coins = {2,5,3,6};
    cout << "Recursive: " << change_rec(n, coins) << endl;
    cout << "Tabulation: " << change_tab(n, coins) << endl;
}
